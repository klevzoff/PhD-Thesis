...
\begin{figure} [htbp]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \begin{semilogxaxis}[ width  = \linewidth,
      ...
      \addplot+[layer1, only marks, mark=*, mark size =1pt,mark options={solid,fill=layer1}]
               table[x=young_Pa, y=elevation_m, col sep=comma]
               {./figs/CrossSection/data/young/layer_1.csv};  
      \addplot+[layer3, only marks, mark=*, mark size =1pt,mark options={solid,fill=layer3}]
               table[x=young_Pa, y=elevation_m, col sep=comma]
               {./figs/CrossSection/data/young/layer_3.csv};   
      \addplot+[layer2, only marks, mark=*, mark size =1pt,mark options={solid,fill=layer2}]
               table[x=young_Pa, y=elevation_m, col sep=comma]
               {./figs/CrossSection/data/young/layer_2.csv};            
      \addplot+[layer4, only marks, mark=*, mark size =1pt,mark options={solid,fill=layer4}]
               table[x=young_Pa, y=elevation_m, col sep=comma]
               {./figs/CrossSection/data/young/layer_4.csv};
      \addplot+[layer5, only marks, mark=*, mark size =1pt,mark options={solid,fill=layer5}]
               table[x=young_Pa, y=elevation_m, col sep=comma]
               {./figs/CrossSection/data/young/layer_5.csv};            
      \addplot+[layer6, only marks, mark=*, mark size =1pt,mark options={solid,fill=layer6}]
               table[x=young_Pa, y=elevation_m, col sep=comma]
...
\paragraph{Mechanics Problem}
A vertical distribution of Young's modulus is prescribed, based on a correlation for uniaxial compressibility as function of depth developed in \cite{Bau2002} and recently used in \cite{Castelletto2017}:
\begin{align}
...
\end{align}
where $\sigma'_z$ is the vertical effective stress and $c_M$ is medium vertical compressibility (both in units of bar), and Poisson ratio is set to $\nu = 0.3$ everywhere.   Additional discontinuity between layers is introduced by adding a different constant value to Young's modulus in each layer, specifically the mean Young's modulus in that layer multiplied by a layer--dependent coefficient.   The resulting distribution spans 3 orders of magnitude over the domain and is depicted in \cref{fig:cross_section_young}.   Left, right and bottom sides of the domain are prescribed to have constrained normal displacement ($\vec{u} \cdot \vec{n} = 0$), while the top side is traction-free ($\tensorTwo{\sigma} \cdot \vec{n} = \vec{0}$).   Dirichlet boundary conditions are prescribed in a symmetric fashion, enabling the use of CG iteration.   The deformation is driven by the forcing term generated by a constant pressure drawdown of $\Delta p = -20$ MPa prescribed via the right-hand side in a small reservoir zone shown in \cref{fig:cross_section_sketch}.   Reference solution for this problem is shown in \cref{fig:cross_section_mech_sol}, along with approximate solution generated by a single (non-iterated) application of the multiscale preconditioner.   As the plots indicate, multiscale solution qualitatively captures the solution shape, but the initial error can be quite large (reaching up to 33\% for $z$-displacement).   This is explained by the use of simple pointwise smoother (symmetric Gauss-Seidel in this case) that needs multiple sweeps to propagate localized right-hand side values of the forcing term.   As we'll see, this does not present a problem in the context of a Krylov solver.

\paragraph{Flow Problem}
The domain is assigned a permeability field generated as follows: a mean permeability is chosen for each layer in the range of $10^{-15}$--$10^{-12}$ m\textsuperscript{2} (1 mD to 1 D), then a high-resolution background Gaussian random field is interpolated onto each mesh and used to assign each layer a random permeability field with the chosen mean and a range of $\pm1$ order of magnitude.   The resulting distribution (shown in \cref{fig:cross_section_perm}) spans approximately 5 orders of magnitude and has both sharp large-scale contrasts across layers and small-scale variations within each layer.   We use TPFA scheme to discretize the system, and both fluid and rock are set to be incompressible, resulting in a steady-state problem with symmetric system matrix, which allows us to use CG solver.   The domain is prescribed no-flow boundary conditions on all sides, with two pressure-controlled wells in the opposite corners of the domain operating at -10 and 10 MPa, respectively (relative to some unspecified in-situ pressure).   Reference and approximate multiscale solutions are shown in \cref{fig:cross_section_flow_sol} where we observe that, similar to mechanics case, the initial solution error is quite large, though qualitatively correct.

\paragraph{Poromechanical Problem}
The coupled problem combines material properties and boundary conditions from the mechanics and flow problems, except that pressure drawdown is not applied and only wells are used to drive the flow and deformation.   A Biot coefficient of 1 and fluid viscosity of $10^{-3}$ Pa$\cdot$s (1 cP) are used, and the time step size is set to $\Delta t = 10^6$ s ($\approx$ 11.6 days), which corresponds to early transient behavior (the characteristic consolidation time for this problem is estimated to be $T_c \approx 10^8$ s.   Unlike the separate mechanics and flow problems, coupled system matrix is not symmetric due to sign and additional scaling of matrix blocks designed to improve system conditioning; therefore GMRES is used as the iterative solver.

\begin{table*}
    \centering
    \caption[2D cross-section test case refinement problem sizes]{2D cross-section test case: refinement levels and corresponding fine- and coarse-scale problem sizes and coarsening ratios}
    \label{tab:cross_section_dims}
    \begin{tabular}{rrrrrrrrrcc}
        \hline\noalign{\smallskip}
        \multirow{2}{*}{$\ell$} & & \multicolumn{2}{c}{fine grid} & & \multicolumn{2}{c}{coarse grid} & & \multicolumn{2}{c}{coarsening ratio} \\
        \noalign{\smallskip}\cline{3-4} \cline{6-7} \cline{9-10}\noalign{\smallskip}
        & & \# cell & \# node & & \# cell & \# node & & flow & mech \\
        \hline\noalign{\smallskip}
        0 & &    11,879 &   6,119 & &    32 &    65 & & 371.2 & 94.1 \\
        1 & &    23,390 &  11,947 & &    64 &   128 & & 365.5 & 93.3 \\
        2 & &    46,932 &  23,817 & &   128 &   256 & & 366.7 & 93.0 \\
        3 & &    93,129 &  47,085 & &   256 &   512 & & 363.8 & 92.0 \\
        4 & &   186,940 &  94,165 & &   512 & 1,021 & & 365.1 & 92.2 \\
        5 & &   372,359 & 187,210 & & 1,024 & 2,038 & & 363.6 & 91.9 \\
        6 & &   745,904 & 374,346 & & 2,048 & 4,078 & & 364.2 & 91.8 \\
        7 & & 1,490,288 & 747,199 & & 4,096 & 8,162 & & 363.8 & 91.5 \\
        \hline\noalign{\smallskip}
    \end{tabular}
\end{table*}
...
The second test case represents a 3D porous media domain consisting of 4 distinct layers with two intersecting faults (which, once again, are considered inactive).   The domain is discretized with an unstructured tetrahedral mesh with various levels of refinement, summarized in \cref{tab:mazumodel2_dims} along with coarse grid sizes for the 2-level scheme.    Here we use two different sets of coarse grid for mechanics and flow problems, including in the coupled poromechanics test.   Note that effective coarsening ratio $C_r^d$ is much lower for the node-based degrees-of-freedom (mechanics), despite the coarse level grid having a lower resolution --- it is typical for unstructured agglomerates to have a large number of nodes per cell in three dimensions.

\paragraph{Flow problem}
Similar to the previous case, we consider an incompressible flow problem.   The permeability field of top 15 layers of the SPE10 Comparative Solution Project's model 2 \cite{Christie2001} are is mapped to the tetrahedral mesh using nearest neighbor interpolation at every resolution level (shown in \cref{fig:mazumodel2_setup_permx,fig:mazumodel2_setup_permz}), preserving sharp contrasts and anisotropy present in source data (filtering is applied to pad zero values to a very low threshold value).   The boundary conditions are no-flow, and a 5-spot well pattern is modeled with 4 vertical injector wells a small distance away from $x$-$y$ plane corners and a producer well in the center of the domain (see \cref{fig:mazumodel2_setup_flow_sketch}).   All wells are completed throughout the middle (reservoir) layer, and an approximate Peaceman well transmissibility is computed for the intersected tetrahedral cells.
...
\begin{align}
    E = E_{\max} + (\bar{E} - E_{\max})\frac{\log_{10}(k_x) - \min(\log_{10}(k_x))}{\Mean(\log_{10}(k_x)) - \min(\log_{10}(k_x))}
\end{align}
where average $\bar{E} = 5$ GPa and $E_{\max} = 1.5 \bar{E} = 7.5$ GPa, resulting in a distribution shown in \cref{fig:mazumodel2_setup_young}.   Like in the 2D cross-section case, the boundary conditions include zero normal displacement on the sides and bottom and zero traction on the top surface, and a pressure drop of $\Delta p = -20$ MPa in a small zone of the reservoir layer (seen in \cref{fig:mazumodel2_setup_mech_sketch}).

\begin{figure} [htbp]
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth]{figs/MazuModel2/mech_sketch}
        \caption{Mechanics problem setup}
        \label{fig:mazumodel2_setup_mech_sketch}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=0.9\linewidth]{figs/MazuModel2/flow_sketch}
        \caption{Flow problem setup}
        \label{fig:mazumodel2_setup_flow_sketch}
    \end{subfigure}
    \par\bigskip
    \begin{subfigure}[t]{0.3\textwidth}
        \includegraphics[width=\linewidth]{figs/MazuModel2/permx}
        \caption{Log-scale horizontal permeability, [m\textsuperscript{2}]}
        \label{fig:mazumodel2_setup_permx}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.3\textwidth}
        \includegraphics[width=\linewidth]{figs/MazuModel2/permz}
        \caption{Log-scale vertical permeability, [m\textsuperscript{2}]}
        \label{fig:mazumodel2_setup_permz}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.3\textwidth}
        \includegraphics[width=\linewidth]{figs/MazuModel2/young}
        \caption{Young's modulus, [Pa]}
        \label{fig:mazumodel2_setup_young}
    \end{subfigure}
    \caption[3D synthetic reservoir test case setup]{3D synthetic reservoir test case setup.}
    \label{fig:mazumodel2_setup}
\end{figure}

\begin{table}
    \centering
    \caption[3D synthetic reservoir test case refinement problem sizes]{3D synthetic reservoir test case: refinement levels and corresponding fine- and coarse-scale problem sizes and coarsening ratios}
    \label{tab:mazumodel2_dims}
    \begin{tabular}{rrrrrrrrrrrrcc}
      ...
      & & \# cell & \# node & & \# cell & \# node & $C_r^d$ & & \# cell & \# node & $C_r^d$ \\
      \hline\noalign{\smallskip}
